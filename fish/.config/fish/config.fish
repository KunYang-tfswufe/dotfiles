...
# 函数 s_phone1: 快速 SSH 连接到手机1
function s_phone1 --description "通过 MAC 地址发现并 SSH 连接到手机1"
    # 1. 调用脚本获取 IP
    ...
    # 3. 执行连接命令 (!! 注意下面的用户名 !!)
    echo "==> 发现手机1 IP: $phone1_ip, 正在连接..."
    kitten ssh "YOUR_PHONE_USER@$phone1_ip"
end

# 函数 s_phone2: 快速 SSH 连接到手机2
function s_phone2 --description "通过 MAC 地址发现并 SSH 连接到手机2"
    # 1. 调用脚本获取 IP
    ...
    # 3. 执行连接命令 (!! 注意下面的用户名 !!)
    echo "==> 发现手机2 IP: $phone2_ip, 正在连接..."
    kitten ssh "YOUR_PHONE_USER@$phone2_ip"
end
