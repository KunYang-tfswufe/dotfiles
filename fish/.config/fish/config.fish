...
# =============================================================================
#  为手机添加的 SSH 连接函数 (V3 - 带交互式提醒)
# =============================================================================

# 函数 s_phone1: 快速 SSH 连接到手机1 (Termux)
function s_phone1 --description "提醒后通过 MAC 地址发现并 SSH 连接到手机1 (Termux)"
    # 使用 Fish 内置的 set_color 命令来让提示更醒目
    set_color yellow
    echo "------------------ [操作提醒] ------------------"
    echo "请在安卓手机的 Termux 中, 确保已完成以下操作:"
    echo "  1. 运行 'sshd' 命令来启动 SSH 服务."
    echo "  2. (如果首次使用) 运行 'passwd' 命令来设置连接密码."
    echo "-----------------------------------------------"
    set_color normal # 恢复正常颜色

    # 使用 read 命令暂停脚本，并等待用户按 Enter 键
    read --prompt-str "完成后, 请按 Enter键 继续连接 (按 Ctrl+C 取消)..."
