...
# -----------------------------------------------------------------------------
#  为手机添加的 SSH 连接函数 (V2 - Termux 修正版)
# -----------------------------------------------------------------------------

# 函数 s_phone1: 快速 SSH 连接到手机1 (Termux)
function s_phone1 --description "通过 MAC 地址发现并 SSH 连接到手机1 (Termux)"
    ...
    # 【核心修正】添加 -p 8022，并且因为 Termux 不关心用户名，所以直接省略
    kitten ssh -p 8022 "$phone1_ip"
end

# 函数 s_phone2: 快速 SSH 连接到手机2 (Termux)
function s_phone2 --description "通过 MAC 地址发现并 SSH 连接到手机2 (Termux)"
